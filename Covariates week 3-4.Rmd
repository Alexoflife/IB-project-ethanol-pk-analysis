...
p <- plot_model(fit_AUC_mod_with_food, show.values = TRUE,  show.intercept = TRUE, value.offset = 0.3, axis.labels = c("Dose (g per kg) x Route (PO)", "Food status (fed)", "Route (PO)", "Dose (g per kg)","Intercept")) +
...
Cmax_lm2_v1 <- lm(log(Cmax) ~ dose_g_per_kg + route + food + site, data = order_0_df)
...
Cmax_lm2_v2 <- lm(log(Cmax) ~ dose_g_per_kg + route + food + sex + age, data = order_0_df)
...
Cmax_lm2_v3 <- lm(log(Cmax) ~ dose_g_per_kg + age + bsa_m2 + lbm_kg, data = order_0_df)
...
```{r}
fit_Beta_1 <- lm(Beta ~ dose_g_per_kg * route + food, data = order_0_df)
...
labs(title = "Regression coefficients \n for Beta with 95% CI", 
       subtitle = "Beta ~ dose_g_per_kg * route + feed") +

...
Yelland_2008_clean %>% 
  ...
  mutate(occasion = occasion %>% as.factor) %>% 
  ggplot() +
  geom_line(aes(x = time_min, y = conc_g_per_L, group = occasion, color = occasion), linewidth = 2) +
  geom_point(aes(x = time_min, y = conc_g_per_L)) +
  facet_wrap(subject_id~.)+
  scale_x_continuous(breaks = seq(0, 420, by = 50))+
  scale_y_continuous(breaks = seq(0, 1, by = 0.25))+
    labs(title = "Ethanol concenctration curves by subject", x = "Time, min", y = "Concentration, g/l", color = "Occasion", caption = "Yelland-2008")+
  theme(legend.position = "top")
```
...
```{r Correlation, fig.height=8, fig.width=9, dpi = 200}
pk_num <- order_0_df %>% 
  select(where(is.numeric)) %>% 
  rename(`Lean body weight` = lbm_kg,
         `Body surface area` = bsa_m2,
         `Body mass index` = bmi,
         `Body height` = bh_cm,
         `Age` = age,
         `Body weight` = wt_kg,
...
# cor_mat <- cor(pk_num, use = "pairwise.complete.obs", method = "pearson")

ggcorr(pk_num, method = c("pairwise.complete.obs", "pearson"), label = TRUE, hjust = 0.78)
...
```{r}
ggpredict(t_max_full_m, terms = c("dose_g_per_kg", "route"), ci_level = 0) %>% plot(show_data = TRUE, log_y = FALSE, jitter = .01) + labs(subtitle = "Adjusted R-squared:  0.497")
```

```{r, fig.height=8, fig.width=8, dpi=300}
plot_model(t_max_full_m, show.values = TRUE,  show.intercept = TRUE, value.offset = 0.3, vcov.fun = vcovHC, vcov.args = "HC3", axis.labels = c("Dose (g per kg) x food status: \"Fed\"", "Weight, kg", "Route (PO)", "Food status: \"Fed\"", "Dose (g per kg)", "Intercept"), title = "Regression coeffecients with 95% CI") +
  theme_nice()
```

...
CVInter, CVintra рассчитывались на основе среднего квадрата ошибки, определяемого при ANOVA логарифмически преобразованных данных

``` {r log-data}
pk_params_log <- pk_params %>% 
  select(-subject_id, -occasion, -Dose) %>% 
  mutate(across(everything(), log))

pk_params_log$occasion <- pk_params$occasion

pk_params_log <- pk_params_log %>% 
  relocate(occasion, .after = subject_id)
```


```{r log-anova}
AOV_calc <- function(df) {
  aov_var <- aov(value ~ factor(subject_id) + factor(occasion),
                data = df)
  aov_var <- summary(aov_var)
  return(aov_var)
}

pk_params_cv <- pk_params_log %>% 
  pivot_longer(cols = -c(subject_id, occasion), names_to = "variable") %>% 
  group_by(variable) %>% 
  summarize(
    MSb = AOV_calc(cur_data())[[1]]["Mean Sq"][[1]][1],
    MSw = AOV_calc(cur_data())[[1]]["Mean Sq"][[1]][3],
    Vb = (MSb - MSw)/4,
    Vw = MSw,
    Between_subj_var = (Vb/(Vb + Vw)) * 100,
    Within_subj_var = (Vw/(Vb + Vw)) * 100,
    CVintra = sqrt(exp(AOV_calc(cur_data())[[1]]["Mean Sq"][[1]][3]) - 1) * 100,
    CVinter = sqrt(exp(MSb) - 1) * 100,
    CVtotal = CVintra + CVinter
  )
```

Визуализация отклонения от среднего для каждого субъекта в исследовании

``` {r idmeans_vis, fig.height = 10, fig.width = 15, dpi = 300}
pk_params_means <- pk_params %>% 
  group_by(subject_id) %>% 
  summarise(
    C_max = mean(C_max),
    T_max = mean(T_max),
    AUC_linear = mean(AUC_linear),
    Beta_LM = mean(Beta_LM),
    C0_LM = mean(C0_LM),
    Vd = mean(Vd))

temp_2 <- ggplot(pk_params_means) +
  labs(title = " ",
       x = "",
       y = "")+
  theme(axis.text = element_text(size = 11),
...
mean_var_func <- function(var,
                      mapping = aes(x = as.factor(subject_id), y = .data[[var]]),
                          ...){
  mean_val <- mean(pk_params_means[[var]])
  mean_var <- pk_params_means %>% 
    list(
      geom_col(mapping = mapping, fill = "skyblue", colour = "gray18"),
      geom_hline(yintercept = mean_val, colour = "blue", linetype = "dashed"))}

variables = c(pk_params_summ$variable[1:4], pk_params_summ$variable[6:7])

mean_var <- map(variables, 
                \(x) temp_2+
                  mean_var_func(var=x)+
                  scale_x_discrete(name = paste0("Параметр: ", x)))

wrap_plots(mean_var) &
  plot_annotation(title = "Отклонение от среднего для каждого субъекта") 
